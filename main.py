...
def main():
    ...
    for (t_key, t_val), (e_key, e_val) in zip(train_metrics.items(), eval_metrics.items()):
        eval_metrics[e_key] = {
            t_key: t_val,
            **e_val
        }
    save_results(eval_metrics, exp_name, str(results_root))
    logger.info(f"Spent {(time.time()-st_time)/3600:.1f}h: Experiment {exp_name} completed. Combined metrics: {eval_metrics}")
