...
from .utils import load_naip_tile, augment_naip_tile
...
class RandomSplitDataset(Dataset):
    def __init__(self, cfg: dict, split: str = 'train'):
        ...
        train_ratio = cfg['data']['split']['random']['train_ratio']  # e.g., 0.8
        ...
        if self.split == 'test':
            # Save the TreeType corresponding to each tile for comparison accuracy.
    ...
    def __getitem__(self, idx: int):
        ...
        # 2. Load auxiliary label
        with rasterio.open(os.path.join(self.edge_base, filename.replace(".tif", "_edge.tif"))) as f:
            edge_label = f.read(1)
        with rasterio.open(os.path.join(self.dist_base, filename.replace(".tif", "_dist.tif"))) as f:
            dist_label = f.read(1)
        ...
        # Data Augmentation
        if self.split == 'train':
            # Stack：Mask, Edge, PointDist, BoundaryDist
            combined_labels = np.stack([label, edge_label, dist_label, boundary_dist], axis=0)
            naip, combined_labels, no_data_mask = augment_naip_tile(
                naip, combined_labels, no_data_mask, self.cfg['data']['augmentation']
            )
